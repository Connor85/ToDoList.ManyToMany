...
namespace ToDoList.Models
{
  public class Category
  {
    ...
    public List<Item> GetItems()
    {
      ...
      conn.Open();
      var cmd = conn.CreateCommand() as MySqlCommand;
      cmd.CommandText = @"SELECT items.* FROM categories JOIN categories_items ON (categories.id = categories_items.category_id) WHERE categories.id = @CategoryId;";
      ...
      foreach (int itemId in itemIds)
      {
        ...
        while(itemQueryRdr.Read())
        {
          ...
          string   newDueDate=itemQueryRdr.GetString(2);
          Item foundItem = new Item(itemDescription, newDueDate,thisItemId);
          items.Add(foundItem);
        }
